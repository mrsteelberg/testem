...
Server.prototype = {
    ...
    , configureProxy: function() {
        var exp = this.exp
        var proxyConfig = this.app.config.get("proxy")

        var proxyRequest = function( path, topUrl, req, res, next) {
            var uri = req.url.substring(path.length)
            var url = topUrl +  uri
        ...
        exp.use(function(req, res, next) {
            for(var path in proxyConfig){
                if(req.url.substring(0, path.length) === path) {
                    proxyRequest(path, proxyConfig[path], req, res, next)
                    return
                }
